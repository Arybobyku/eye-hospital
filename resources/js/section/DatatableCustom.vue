						...
						<td>
							<center>
								<span style="position: relative;">
									<button class="tooltip btn-menutable" style="padding-top: 4px;" v-on:click="getbutton($event, index, 2)">
										<vue-feather type="cpu"></vue-feather>
									</button>
									<ul class="hidden-menu" v-if="row[2].show">
										<template v-for="btn in row[2].value">
											<li v-if="btn.show">
...
.nested-scroll {
  ...
  overflow-y: auto;
  border: 1px solid #ddd; /* biar keliatan area scroll */
  background: #fafafa;   /* beda warna supaya jelas */
}
