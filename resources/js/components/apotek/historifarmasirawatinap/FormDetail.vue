								...
								<table class="table">
									...
									<tbody>
										<tr v-for="(item, index) in listobat">
											...
											<template v-if="detail.status_kasir == 'Belum Bayar'">
												<td>
													<input type="number" :value="item.jumlah_kecil" style="width: 100%;" v-on:keyup="ubah($event, item, index)" />
												</td>
...
export default {
	...
	data:function() { return { 
		listdata: [], tmplistdata:[], templistobat:[], listobat: [], listobatracikan: [], tempobat: null,
	...
	methods: {
		...
		ubah:function(event, item, index) {
			...
			if (number != '') {
				let hitungan = parseFloat(number * item.hja_resep);
				vm.listobat[index].jumlah_kecil = number;
				vm.listobat[index].total = Math.floor(hitungan);
			}
		...
		parsingForm:function() { 
			vm.$emit('parsingForm', vm.parsekelurahan(vm.form, vm.detail, vm.listobat), 'editobat'); 
		},
