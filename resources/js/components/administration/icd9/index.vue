...
<FormIcd9 ref="FormIcd9" @dialog="dialog" @parsingForm="parsingForm"></FormIcd9>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormIcd9: defineAsyncComponent(() => import('./FormIcd9.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/icd9/list',
				add: '/administration/icd9/add',
				edit: '/administration/icd9/edit',
				update: '/administration/icd9/update',
				remove: '/administration/icd9/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama ICD 9', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormIcd9.aturulang();
				vm.position = "adddata";
				vm.$refs.FormIcd9.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormIcd9.aturulang();
				vm.position = "editdata";
				vm.$refs.FormIcd9.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formicd9'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formicd9') { vm.$refs.FormIcd9.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formicd9'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formicd9'); vm.$refs.FormIcd9.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formicd9'); }
		...
		runconfirm: function (posisi) {
			if (posisi == 'formicd9') { vm.loadingModal('formicd9'); }
