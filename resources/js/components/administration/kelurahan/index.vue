...
<FormKelurahan ref="FormKelurahan" @dialog="dialog" @parsingForm="parsingForm"></FormKelurahan>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormKelurahan: defineAsyncComponent(() => import('./FormKelurahan.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/kelurahan/list',
				add: '/administration/kelurahan/add',
				edit: '/administration/kelurahan/edit',
				update: '/administration/kelurahan/update',
				remove: '/administration/kelurahan/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama_kecamatan', label: 'Nama Kecamatan', type: 'text', search: true, close: false, button: false },
			{ value: 'nama', label: 'Nama Kelurahan', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormKelurahan.aturulang();
				vm.position = "adddata";
				vm.$refs.FormKelurahan.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormKelurahan.aturulang();
				vm.position = "editdata";
				vm.$refs.FormKelurahan.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formkelurahan'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formkelurahan') { vm.$refs.FormKelurahan.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formkelurahan'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formkelurahan'); vm.$refs.FormKelurahan.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formkelurahan'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formkelurahan');
				vm.$refs.FormKelurahan.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormKelurahan.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formkelurahan') { vm.loadingModal('formkelurahan'); }
