...
<FormKabkota ref="FormKabkota" @dialog="dialog" @parsingForm="parsingForm"></FormKabkota>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormKabkota: defineAsyncComponent(() => import('./FormKabkota.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/kabkota/list',
				add: '/administration/kabkota/add',
				edit: '/administration/kabkota/edit',
				update: '/administration/kabkota/update',
				remove: '/administration/kabkota/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama_provinsi', label: 'Nama Provinsi', type: 'text', search: true, close: false, button: false },
			{ value: 'nama', label: 'Nama Kabupaten/Kota', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormKabkota.aturulang();
				vm.position = "adddata";
				vm.$refs.FormKabkota.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormKabkota.aturulang();
				vm.position = "editdata";
				vm.$refs.FormKabkota.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formkabkota'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formkabkota') { vm.$refs.FormKabkota.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formkabkota'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formkabkota'); vm.$refs.FormKabkota.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formkabkota'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formkabkota');
				vm.$refs.FormKabkota.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormKabkota.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formkabkota') { vm.loadingModal('formkabkota'); }
