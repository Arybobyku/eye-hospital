...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/runningimage/list',
				add: '/administration/runningimage/add',
				block: '/administration/runningimage/block',
			}, url: '', data: null
		},
		column: [
			{ value: 'content', label: 'Data Image', type: 'text', search: false, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		btnhtml:function(_item, _index) {
			let str = [
				{ 
					icon: 'x-circle', color: 'btn-danger', posisi: 'block', tooltip: 'Delete Gambar', item: _item, index: _index, 
					show: true },
			]
			return str;
		},
		...
		converter: function (data, index, column, identity) {
			...
			else if (identity == 'content') { _tmp = { value: vm.contents(data), ishtml: 'html', style: '' }; }
		...
		tablebutton:function(posisi, data, index) {
			...
			// }
				...
				vm.dialog('Yakin ingin menghapus data yang terpilih dihalaman ini.', 'Ya, hapus data', 'blockdata');
			}
		...
		*************************************************************************************************************************/
		...
		message: function (position, active) {
			if (position == 'error') {
				...
				else if (vm.position == 'blockdata') { vm.notification('Penghapusan data gagal diproses.', 3000, position); }
				...
				else if (vm.position == 'blockdata') { vm.notification('Penghapusan data berhasil diproses.', 3000, position); }
