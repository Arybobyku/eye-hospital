...
<FormTindakanBedah ref="FormTindakanBedah" @dialog="dialog" @parsingForm="parsingForm"></FormTindakanBedah>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormTindakanBedah: defineAsyncComponent(() => import('./FormTindakanBedah.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/tindakanbedah/list',
				add: '/administration/tindakanbedah/add',
				edit: '/administration/tindakanbedah/edit',
				update: '/administration/tindakanbedah/update',
				remove: '/administration/tindakanbedah/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama Tindakan', type: 'text', search: true, close: false, button: false },
			{ value: 'jenis', label: 'Jenis Tindakan', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormTindakanBedah.aturulang();
				vm.position = "adddata";
				vm.$refs.FormTindakanBedah.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormTindakanBedah.aturulang();
				vm.position = "editdata";
				vm.$refs.FormTindakanBedah.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formtindakanbedah'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formtindakanbedah') { vm.$refs.FormTindakanBedah.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formtindakanbedah'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formtindakanbedah'); vm.$refs.FormTindakanBedah.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formtindakanbedah'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formtindakanbedah');
				vm.$refs.FormTindakanBedah.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormTindakanBedah.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formtindakanbedah') { vm.loadingModal('formtindakanbedah'); }
