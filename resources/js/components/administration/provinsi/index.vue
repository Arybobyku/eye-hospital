...
<FormProvinsi ref="FormProvinsi" @dialog="dialog" @parsingForm="parsingForm"></FormProvinsi>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormProvinsi: defineAsyncComponent(() => import('./FormProvinsi.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/provinsi/list',
				add: '/administration/provinsi/add',
				edit: '/administration/provinsi/edit',
				update: '/administration/provinsi/update',
				remove: '/administration/provinsi/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama Provinsi', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormProvinsi.aturulang();
				vm.position = "adddata";
				vm.$refs.FormProvinsi.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormProvinsi.aturulang();
				vm.position = "editdata";
				vm.$refs.FormProvinsi.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formprovinsi'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formprovinsi') { vm.$refs.FormProvinsi.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formprovinsi'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formprovinsi'); vm.$refs.FormProvinsi.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formprovinsi'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formprovinsi');
				vm.$refs.FormProvinsi.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormProvinsi.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formprovinsi') { vm.loadingModal('formprovinsi'); }
