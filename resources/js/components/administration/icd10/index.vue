...
<FormIcd10 ref="FormIcd10" @dialog="dialog" @parsingForm="parsingForm"></FormIcd10>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormIcd10: defineAsyncComponent(() => import('./FormIcd10.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/icd10/list',
				add: '/administration/icd10/add',
				edit: '/administration/icd10/edit',
				update: '/administration/icd10/update',
				remove: '/administration/icd10/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama ICD 10', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormIcd10.aturulang();
				vm.position = "adddata";
				vm.$refs.FormIcd10.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormIcd10.aturulang();
				vm.position = "editdata";
				vm.$refs.FormIcd10.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formicd10'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formicd10') { vm.$refs.FormIcd10.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formicd10'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formicd10'); vm.$refs.FormIcd10.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formicd10'); }
		...
		runconfirm: function (posisi) {
			if (posisi == 'formicd10') { vm.loadingModal('formicd10'); }
