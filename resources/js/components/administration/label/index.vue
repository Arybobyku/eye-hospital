...
<FormLabel ref="FormLabel" @dialog="dialog" @parsingForm="parsingForm"></FormLabel>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormLabel: defineAsyncComponent(() => import('./FormLabel.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/label/list',
				add: '/administration/label/add',
				edit: '/administration/label/edit',
				update: '/administration/label/update',
				remove: '/administration/label/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'based', label: 'Group', type: 'text', search: true, close: false, button: false },
			{ value: 'nama', label: 'Nama Label', type: 'text', search: true, close: false, button: false },
			{ value: 'icon', label: 'Icon Label', type: 'text', search: true, close: false, button: false },
			{ value: 'link', label: 'Link Label', type: 'text', search: true, close: false, button: false },
			{ value: 'posisi', label: 'Posisi ke', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormLabel.aturulang();
				vm.position = "adddata";
				vm.$refs.FormLabel.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormLabel.aturulang();
				vm.position = "editdata";
				vm.$refs.FormLabel.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formlabel'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formlabel') { vm.$refs.FormLabel.loaderprocess();  }
		},
				...
				for (let j = 0; j < vm.column.length; j++) { 
					col.push(
						vm.converter(
							data[i], 
							i, 
							data[i][vm.column[j].value] ? data[i][vm.column[j].value] :vm.column[j].value, 
							vm.column[j].value
						)
					); 
				} 
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formlabel'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formlabel'); vm.$refs.FormLabel.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formlabel'); }
		...
		runconfirm: function (posisi) {
			if (posisi == 'formlabel') { vm.loadingModal('formlabel'); }
