...
<FormKecamatan ref="FormKecamatan" @dialog="dialog" @parsingForm="parsingForm"></FormKecamatan>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormKecamatan: defineAsyncComponent(() => import('./FormKecamatan.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/kecamatan/list',
				add: '/administration/kecamatan/add',
				edit: '/administration/kecamatan/edit',
				update: '/administration/kecamatan/update',
				remove: '/administration/kecamatan/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama_kab_kota', label: 'Nama Kabupaten/Kota', type: 'text', search: true, close: false, button: false },
			{ value: 'nama', label: 'Nama Kecamatan', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormKecamatan.aturulang();
				vm.position = "adddata";
				vm.$refs.FormKecamatan.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormKecamatan.aturulang();
				vm.position = "editdata";
				vm.$refs.FormKecamatan.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formkecamatan'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formkecamatan') { vm.$refs.FormKecamatan.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formkecamatan'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formkecamatan'); vm.$refs.FormKecamatan.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formkecamatan'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formkecamatan');
				vm.$refs.FormKecamatan.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormKecamatan.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formkecamatan') { vm.loadingModal('formkecamatan'); }
