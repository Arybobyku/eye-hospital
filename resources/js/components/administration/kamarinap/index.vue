...
<FormKamarInap ref="FormKamarInap" @dialog="dialog" @parsingForm="parsingForm"></FormKamarInap>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormKamarInap: defineAsyncComponent(() => import('./FormKamarInap.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/kamarinap/list',
				add: '/administration/kamarinap/add',
				edit: '/administration/kamarinap/edit',
				update: '/administration/kamarinap/update',
				remove: '/administration/kamarinap/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama_jenis_kamar', label: 'Jenis Kamar', type: 'text', search: true, close: false, button: false },
			{ value: 'nama', label: 'Nama Kamar', type: 'text', search: true, close: false, button: false },
			{ value: 'lantai', label: 'Lantai', type: 'text', search: true, close: false, button: false },
			{ value: 'jumlah_bed', label: 'Jumlah Bed', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormKamarInap.aturulang();
				vm.position = "adddata";
				vm.$refs.FormKamarInap.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormKamarInap.aturulang();
				vm.position = "editdata";
				vm.$refs.FormKamarInap.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formkamarinap'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formkamarinap') { vm.$refs.FormKamarInap.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formkamarinap'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formkamarinap'); vm.$refs.FormKamarInap.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formkamarinap'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formkamarinap');
				vm.$refs.FormKamarInap.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormKamarInap.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formkamarinap') { vm.loadingModal('formkamarinap'); }
