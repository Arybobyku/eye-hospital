...
<FormJenisKamar ref="FormJenisKamar" @dialog="dialog" @parsingForm="parsingForm"></FormJenisKamar>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormJenisKamar: defineAsyncComponent(() => import('./FormJenisKamar.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/jeniskamar/list',
				add: '/administration/jeniskamar/add',
				edit: '/administration/jeniskamar/edit',
				update: '/administration/jeniskamar/update',
				remove: '/administration/jeniskamar/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Jenis Kamar', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormJenisKamar.aturulang();
				vm.position = "adddata";
				vm.$refs.FormJenisKamar.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormJenisKamar.aturulang();
				vm.position = "editdata";
				vm.$refs.FormJenisKamar.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formjeniskamar'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formjeniskamar') { vm.$refs.FormJenisKamar.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formjeniskamar'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formjeniskamar'); vm.$refs.FormJenisKamar.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formjeniskamar'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formjeniskamar');
				vm.$refs.FormJenisKamar.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormJenisKamar.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formjeniskamar') { vm.loadingModal('formjeniskamar'); }
