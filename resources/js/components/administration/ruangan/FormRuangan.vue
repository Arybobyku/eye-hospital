			...
			<div class="modal-body" v-if="form">
				<div class="grid">
					<div class="col-12">
						...
						<Selected v-on:click="selectbox($event, form.select.jenisruangan.name, form.select.jenisruangan.statics)" 
							:ref="form.select.jenisruangan.name" @selecteditem="selecteditem" @selectclear="selectclear"
							:selection="form.select.jenisruangan"></Selected>
...
import { formruangan } from './FormData.js';
import { parseruangan } from './Attachment.js';
import { arrruangan } from '../../../module/DataArray.js';
...
export default {
	...
	mounted:function() { 
		...
		vm.arr = vm.arrruangan();
		vm.form = vm.formruangan();
	...
	methods: {
		...
		parseruangan, formruangan, arrruangan,
		...
		aturulang: function () { vm.form = vm.formruangan(); },
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parseruangan(vm.form), 'ruangan'); },
		...
		setdataform: function (response) {
			...
			vm.form.select.jenisruangan.value = response.data.data.jenis;
			vm.form.select.jenisruangan.label = response.data.data.jenis;
			vm.loaderprocess();
		},
      ...
      vm.$emit('dialog', text, button, 'formruangan');
    },
	}
}
</script>