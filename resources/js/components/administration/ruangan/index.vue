...
<FormRuangan ref="FormRuangan" @dialog="dialog" @parsingForm="parsingForm"></FormRuangan>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormRuangan: defineAsyncComponent(() => import('./FormRuangan.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/ruangan/list',
				add: '/administration/ruangan/add',
				edit: '/administration/ruangan/edit',
				update: '/administration/ruangan/update',
				remove: '/administration/ruangan/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama Ruangan', type: 'text', search: true, close: false, button: false },
			{ value: 'jenis', label: 'Jenis Ruangan', type: 'text', search: true, close: false, button: false },
			{ value: 'lantai', label: 'Lantai ke', type: 'number', search: true, close: false, button: false },
			{ value: 'jumlah_bed', label: 'Jumlah Tempat Tidur', type: 'number', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormRuangan.aturulang();
				vm.position = "adddata";
				vm.$refs.FormRuangan.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormRuangan.aturulang();
				vm.position = "editdata";
				vm.$refs.FormRuangan.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formruangan'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formruangan') { vm.$refs.FormRuangan.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formruangan'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formruangan'); vm.$refs.FormRuangan.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formruangan'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formruangan');
				vm.$refs.FormRuangan.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormRuangan.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formruangan') { vm.loadingModal('formruangan'); }
