...
<FormTindakanNonBedah ref="FormTindakanNonBedah" @dialog="dialog" @parsingForm="parsingForm"></FormTindakanNonBedah>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormTindakanNonBedah: defineAsyncComponent(() => import('./FormTindakanNonBedah.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/administration/tindakannonbedah/list',
				add: '/administration/tindakannonbedah/add',
				edit: '/administration/tindakannonbedah/edit',
				update: '/administration/tindakannonbedah/update',
				remove: '/administration/tindakannonbedah/remove',
			}, url: '', data: null
		},
		column: [
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormTindakanNonBedah.aturulang();
				vm.position = "adddata";
				vm.$refs.FormTindakanNonBedah.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormTindakanNonBedah.aturulang();
				vm.position = "editdata";
				vm.$refs.FormTindakanNonBedah.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formtindakannonbedah'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formtindakannonbedah') { vm.$refs.FormTindakanNonBedah.loaderprocess();  }
		},

		parsingForm:function(data, key) {
			vm.attach.data = data;
			if (key == 'tindakannonbedah') {
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formtindakannonbedah'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formtindakannonbedah'); vm.$refs.FormTindakanNonBedah.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formtindakannonbedah'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formtindakannonbedah');
				vm.$refs.FormTindakanNonBedah.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormTindakanNonBedah.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formtindakannonbedah') { vm.loadingModal('formtindakannonbedah'); }
