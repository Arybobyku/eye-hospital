...
<DetailCPPT ref="DetailCPPT" @dialog="dialog" @parsingForm="parsingForm"></DetailCPPT>
...
export default {
	...
	components: { toast, Swal, 
		...
		DetailCPPT: defineAsyncComponent(() => import('./DetailCppt.vue')),
	},
	...
	methods: {
		...
		*************************************************************************************************************************/
		btnhtml:function(_item, _index) {
			let str = [
				{ icon: 'arrow-up', color: 'btn-success', posisi: 'rawatjalan', tooltip: 'Detail CPPT', item: _item, index: _index, show: true },
			]
			return str;
		},
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'rawatjalan') {
				...
				vm.$refs.DetailCPPT.show('modalrawatjalan', 'Resume Medis Rawat Jalan', data.pasien_uuid);
				setTimeout(() => { vm.loadingModal('rawatjalan'); }, 250, this);
				vm.$refs.DetailCPPT.setdataform(data);
			}
		...
		loadingModal: function (position) { 
			if (position == 'rawatjalan') { vm.$refs.DetailCPPT.loaderprocess();  }
