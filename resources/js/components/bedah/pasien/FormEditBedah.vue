	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-semi-besar"
			...
			<div class="modal-header">
				...
				<h2 v-if="form">Ganti Paket Bedah</h2>
			</div>
...
import { formeditbedah } from './FormData.js';
import { parseeditbedah } from './Attachment.js';
...
export default {
	...
	methods: {
		...
		parseeditbedah, formeditbedah, arrtransfer,
		...
		aturulang: function () { vm.form = vm.formeditbedah(); vm.listdata = []; },
		...
		parsingForm:function() { 
			vm.$emit('parsingForm', vm.parseeditbedah(vm.form), 'addbedah'); 
		},
	...
	// 	},
		...
		setdataform: function (response) {
			...
			if (response.data.data) {
				if (response.data.data.namaPaketBedah != '-' && response.data.namaPaketBedah != '') {
					vm.form.select.paketbedah.value = response.data.namaPaketBedah;
					vm.form.select.paketbedah.label = response.data.namaPaketBedah;
				} else {
		...
		dialog:function(){
			let text = '', button = '';
			text = 'Yakin ingin memilih paket bedah';
			button = 'Ya, pilih Paket Bedah';
      vm.$emit('dialog', text, button, 'formeditbedah');
    },
	}
}
</script>