    ...
    <div class="grid" v-if="form && activetab">
            ...
            <table class="table" style="margin-top: 10px;">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: center;"><input class="checkbox" type="checkbox" :checked="os"
                                value="os" style="cursor: pointer;"> </td>
                    </tr>
                    <tr>
        ...
        <div class="col-12">
            <table class="table">
                <thead>
                    <tr>
                        <th>Nama Tindakan untuk Form</th>
                        <th>#</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(item, index) in listtindakan" v-if="listtindakan.length > 0">
                        <td>{{ item . namatindakan }}</td>
                        <td>
                            <button v-if="item.default != 'Ya'" class="tooltip btn-danger"
                                v-on:click="removeTindakanOp(index)">
...
<script>
    ...
    import {
        formlaporaninjeksiantivega
    } from './FormData.js';
    import {
        parselaporaninjeksiantivega
    } from './Attachment.js';
    import {
        filterselected,
        hideselected,
    ...
    export default {
        ...
        data: function() {
            return {
                form: null,
                keyform: 'laporaninjeksiantivega',
                os: false,
				od: false,
				listtindakan: [],
                arr: {
        ...
        methods: {
			...
			selecteditem: function (item, key) {
				...
				console.log(key);
				if (key == 'pilihantindakan') {
					console.log("Pilihan Selected");
					console.log(vm.form.select.pilihantindakan.value);
					console.log(item.value);
					let _item = {
						namatindakan: item.value,
					}
					// pengecekan disini
					let itemExists = vm.listtindakan.some(function (tindakan) {
						return tindakan.namatindakan === item.value;
					});
					if (!itemExists) {
						vm.listtindakan.push(_item);
						console.log("Vm List Tindakan");
						console.log(vm.listtindakan);
						console.log(_item);
					} else {
						console.log("Item already exists in listtindakan");
					}
					vm.form.select.pilihantindakan.value = '';
					vm.form.select.pilihantindakan.label = 'Silahkan Pilih';
				}
			},

            removeTindakanOp: function(index) {
                vm.listtindakan.splice(index, 1);
            },
            ...
            setdataform: function(data, tindakan, row ) {
                vm.form.bedah_uuid = row.uuid;

                console.log(tindakan);

                console.log("bedah uuid js");
                ...
                if (data) {
                    ...
                    if (tindakan) {
                        for (let i = 0; i < tindakan.length; i++) {
                            let _item = {
                                namatindakan: tindakan[i].namatindakan,
                            }
                            vm.listtindakan.push(_item);
                        }
                    }
            ...
            parsingForm: function() {
                ...
                vm.$emit('parsingForm', vm.parselaporaninjeksiantivega(vm.form, vm.listtindakan), vm.keyform);
            },
        }
    }
</script>
