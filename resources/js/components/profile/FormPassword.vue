	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-sedang" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			...
			<div class="modal-body" v-if="form">
				<div class="grid">
					<div class="col-12">
						<Inputed :ref="form.passwordlama.name" :form="form.passwordlama"></Inputed>
						<Inputed :ref="form.passwordbaru.name" :form="form.passwordbaru"></Inputed>
						<Inputed :ref="form.confirmpassword.name" :form="form.confirmpassword"></Inputed>
...
import { formpassword } from './FormData.js';
import { parsepassword } from './Attachment.js';
var vm, body;
export default {
	...
	methods: {
		...
		show:function(posisi, title, penggunauuid){ vm.btnlbl = 'Perbaharui Password'; vm.form.penggunauuid = penggunauuid;
		...
		aturulang: function () { vm.form = vm.formpassword(); },
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parsepassword(vm.form), 'password'); },

		loaderprocess:function() { const left = this.$refs.rootmodal.getBoundingClientRect(); vm.$refs.Loader.running(left, 'modal', 250); },

		dialog:function(){
			let text = '', button = '';
			text = 'Yakin ingin memperbaharui data password.';
			button = 'Ya, perbaharui data';
      vm.$emit('dialog', text, button, 'formpassword');
    },
	}
}
</script>