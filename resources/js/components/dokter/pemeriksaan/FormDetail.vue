									...
									<div class="col-12">
										<table class="table embed" style="border: 0;" v-if="pemeriksaanro">
											...
											<tr>
												...
												<td style="text-align: right;">{{ pemeriksaanro.status_fungsional }}
												</td>
											</tr>
											<tr>
											...
											<tr>
												...
												<td style="text-align: right;">{{ pemeriksaanro.nyeri_hilang_bila }}
												</td>
											</tr>
											<tr>
											...
											<tr>
												...
												<td style="text-align: right;">{{
													detail.penyakit_pernah_diderita_lainnya }}</td>
											</tr>
										...
										<table class="table">
											...
											<tbody>
												<tr v-for="(item, index) in listdata" v-if="listdata.length > 0">
													...
													<td style="display:none">{{ item.jenis }}</td>
	...
	data:function() { return { 
		...
		tab: {
			button: [
					...
					{ value: 'rawatinapjalan', label: 'Rawat Inap', class: 'tab-no-active' },
					{ value: 'onedaycare', label: 'One Day Care (Opsional)', class: 'tab-no-active' },
					{ value: 'rawatinap', label: 'Rawat Inap (Opsional)', class: 'tab-no-active' },
			],
			// racikan: false,
			content: { ro: true, pemeriksaan: false, oculardextra: false, ocularsinistra: false, tindakan: false,  rawatinapjalan: false, resep: false,  racikan: false, onedaycare: false, rawatinap: false  }
		},
		typingTimer: null,
	...
	methods: {
		...
		setdataform: function (response) {
			...
			for (let i = 0; i < response.data.layanan.length; i++){
				if (response.data.layanan[i].jenis != '') {
					var jenis = response.data.layanan[i].jenis
					} else {
					var jenis = ''
					}
				let _item = {
