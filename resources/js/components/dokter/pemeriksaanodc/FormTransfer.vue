	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-semi-besar" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			<div class="modal-header">
				...
				<h2 v-if="form">Transfer Pasien ke Dokter Lain</h2>
			</div>
						...
						<table class="table">
							...
							<tbody v-if="listdata.length > 0">
								<tr v-for="(item, index) in listdata">
									...
									<td>{{ item.pemeriksaan_dokter }}</td>
									<td>{{ item.dokter_jam_periksa }}</td>
									<td>{{ item.dokter_jam_selesai }}</td>
									<td>
										<button v-if="item.status_dokter != 'Sudah Diperiksa'" class="tooltip btn-danger" v-on:click="removetransfer(item)">
											<vue-feather type="x-circle"></vue-feather> 
											<span class="tooltiptext">Batalkan Transfer</span>
										</button>
									</td>
								</tr>
							</tbody>
...
import { formtransfer } from './FormData.js';
import { parsetransfer, parsetransferremove } from './Attachment.js';
...
export default {
	...
	methods: {
		...
		parsetransfer, parsetransferremove, formtransfer, arrtransfer,
		...
		aturulang: function () { vm.form = vm.formtransfer(); vm.listdata = []; },
		...
		parsingForm:function() { 
			vm.$emit('parsingForm', vm.parsetransfer(vm.form), 'addtransfer'); 
		},

		removetransfer:function(item) {
			vm.$emit('parsingForm', vm.parsetransferremove(item.pasien_transfer_uuid, item.registrasi_uuid), 'removetransfer'); 
			let text = '', button = '';
      ...
      vm.$emit('dialog', text, button, 'removetransfer');
		...
		dialog:function(){
			let text = '', button = '';
			text = 'Yakin ingin mentransfer pasien ke dokter yang dipilih.';
			button = 'Ya, transfer pasien';
      vm.$emit('dialog', text, button, 'formtransfer');
    },
	}
}
</script>