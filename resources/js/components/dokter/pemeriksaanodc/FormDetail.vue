      ...
      vm.$emit('dialog', text, button, 'formdetail');
		...
		setlocalstorage: function () {
			if (window.localStorage.getItem("version")) { 
				if (parseInt(window.localStorage.getItem("version") + 1) > 1 ) 
				{ let tmp = window.localStorage.getItem("version"); window.localStorage.setItem("version", (parseInt(tmp)+1)); }
			}
			else { window.localStorage.setItem("version", 1); }
		},
			...
			open.onupgradeneeded = (event) => {
				...
				for (let i = 0; i < keys.length; i++) {
					if( db.objectStoreNames.contains(keys[i]) ){
						//alert(db.deleteObjectStore)
						db.deleteObjectStore(keys[i]);
						//if (m) { alert(db.deleteObjectStore(keys[i])) }
					}
				}
			}
			open.onsuccess = function () { open.result.close(); setTimeout(() => { vm.createdIndexDb(response); }, 350, vm); };
