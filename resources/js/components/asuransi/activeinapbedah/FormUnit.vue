	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-sedang" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			<div class="modal-header">
				<button v-on:click="action()">Perbaharui Nominal</button>
				<span class="close" v-on:click="hide()">&times;</span>
				<h2 v-if="form">Nominal Cover Asuransi</h2>
			</div>
			<div class="modal-body" v-if="form">
				<div class="grid">
					<div class="col-12">
						<Inputed :ref="form.coverasuransi.name" :form="form.coverasuransi"></Inputed>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		setdataform: function (response) {
			...
			vm.form.coverasuransi.value = response.data.data.cover_asuransi;
			vm.loaderprocess();
		},
