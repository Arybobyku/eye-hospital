...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/asuransi/inapbedah/list',
				add: '/asuransi/inapbedah/add',
				edit: '/asuransi/inapbedah/edit',
				update: '/asuransi/inapbedah/update',
				approve: '/asuransi/inapbedah/approve',
				deny: '/asuransi/onedaycare/deny',
				diterima: '/asuransi/inapbedah/diterima',
				dikirim: '/asuransi/inapbedah/dikirim',
				detail: '/asuransi/inapbedah/detail',
			}, url: '', data: null
		},
		column: [
			{ value: 'tanggal', label: 'Tanggal Operasi', type: 'text', search: true, close: false, button: false },
			{ value: 'waktu', label: 'Waktu Operasi', type: 'text', search: true, close: false, button: false },
			...
			{ value: 'nama_layanan', label: 'Paket Bedah', type: 'text', search: true, close: false, button: false },
			{ value: 'tarif', label: 'Harga Paket', type: 'text', search: false, close: false, button: false },
			{ value: 'kamar_inap_nama', label: 'Nama Kamar', type: 'text', search: false, close: false, button: false },
			{ value: 'harga_kamar', label: 'Harga Kamar', type: 'text', search: false, close: false, button: false },
			
			{ value: 'posisi', label: 'Status ke/di Asuransi', type: 'text', search: false, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		btnhtml:function(_item, _index) {
			let str = [
				{ icon: 'send', color: 'btn-info', posisi: 'kirim', tooltip: 'Kirim Data ke Asuransi', item: _item, index: _index, 
					show: _item.posisi == 'Permintaan' ? true : false },
				{ icon: 'check-circle', color: 'btn-info', posisi: 'proses', tooltip: 'Asuransi Disetujui', item: _item, index: _index, 
					show: _item.posisi == 'Dikirim' ? true : false },
				{ icon: 'check-circle', color: 'btn-info', posisi: 'deny', tooltip: 'Asuransi Ditolak', item: _item, index: _index, 
					show: _item.posisi == 'Dikirim' ? true : false },
				// { icon: 'book', color: 'btn-info', posisi: 'selesai', tooltip: 'Registrasi Pasien', item: _item, index: _index, 
		...
		tarifs:function(data) { return formatrupiah(data.tarif.toString()); },

		hargakamar:function(data) { return formatrupiah(data.harga_kamar.toString()); },
		...
		converter: function (data, index, column, identity) {
			...
			else if (identity == 'tarif') { _tmp = { value: vm.tarifs(data), ishtml: 'html', style: '' }; }
			else if (identity == 'harga_kamar') { _tmp = { value: vm.hargakamar(data), ishtml: 'html', style: '' }; }
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'proses') {
				...
				vm.dialog('Apakah benar asuransi pasien ini telah disetujui.', 'Ya, sudah disetujui', 'prosesdata');
			}
			else if (posisi == 'deny') {
				vm.position = "denydata";
				vm.attach.data = new FormData();
				vm.attach.data.append('uuid', data.uuid);
				vm.attach.url = vm.attach.link.deny;
				vm.dialog('Apakah benar asuransi pasien ini telah ditolak.', 'Ya, ditolak asuransi', 'denydata');
			}
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'denydata') { vm.$refs.Datatable.skeleton(); }
			else if (vm.position == 'selesaidata') { vm.$refs.Datatable.skeleton(); }
			else if (vm.position == 'kirimdata') { vm.$refs.Datatable.skeleton(); }
		...
		message: function (position, active) {
			if (position == 'error') {
				...
				else if (vm.position == 'denydata') { vm.notification('Proses perubahan data gagal diproses.', 3000, position); }
				else if (vm.position == 'selesaidata') { vm.notification('Proses perubahan data gagal diproses.', 3000, position); }
				else if (vm.position == 'kirimdata') { vm.notification('Proses perubahan data gagal diproses.', 3000, position); }
			}
				...
				else if (vm.position == 'denydata') { vm.notification('Perusahan perubahan data berhasil diproses.', 3000, position); }
				else if (vm.position == 'selesaidata') { vm.notification('Perusahan perubahan data berhasil diproses.', 3000, position); }
				else if (vm.position == 'kirimdata') { vm.notification('Perusahan perubahan data berhasil diproses.', 3000, position); }
			}
		},
		...
		runconfirm: function (posisi) {
			...
			else if (posisi == 'denydata') { vm.$refs.Datatable.skeleton(); }
			else if (posisi == 'selesaidata') { vm.$refs.Datatable.skeleton(); }
			else if (posisi == 'kirimdata') { vm.$refs.Datatable.skeleton(); }
			vm.executions();
		},
