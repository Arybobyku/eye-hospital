...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/customerservices/rawatinap/list',
				registrasi: '/customerservices/rawatinap/registrasi',
				pindahkamar: '/customerservices/rawatinap/pindahkamar'
			}, url: '', data: null
		},
		column: [
				
			{ value: 'tanggal', label: 'Tanggal Pendaftaran', type: 'text', search: false, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		btnhtml:function(_item, _index) {
			let str = [
				{ icon: 'arrow-up', color: 'btn-info', posisi: 'edit', tooltip: 'Register Pasien', item: _item, index: _index, 
				show: _item.kamar_inap_uuid == '-' ? true : false },
				{ icon: 'move', color: 'btn-info', posisi: 'pindahkamar', tooltip: 'Pindah Ruangan', item: _item, index: _index, 
				show: _item.kamar_inap_uuid != '-' ? true : false }
			]
			return str;
		},
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				...
				vm.$refs.FormUnit.aturulang();
				vm.position = "adddata";
				vm.$refs.FormUnit.show('adddata', 'Edit Data', data.uuid, data);
			}
			else if (posisi == 'pindahkamar') {
				vm.$refs.FormUnit.aturulang();
				vm.position = "pindahkamar";
				vm.$refs.FormUnit.show('pindahkamar', 'Pindah Kamar', data.uuid, data);
			}
		},
		...
		parsingForm:function(data, key) {
			vm.attach.data = data;
			if (key == 'unit') {
				if (vm.position == 'adddata') { vm.attach.url = vm.attach.link.registrasi; }
				else if (vm.position == 'pindahkamar') { vm.attach.url = vm.attach.link.pindahkamar; }
			}
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'pindahkamar') { vm.loadingModal('formunit'); }
		...
		berhasil: function (response) {
			...
			if (response.data.data == 'full') {
				vm.loadingModal('formunit');
				vm.notification('Kelas dari kamar yang anda pilih tidak tersedia, alias penuh.', 3000, position);
			}
			else {
				...
				else if (vm.position == 'pindahkamar') {
					vm.loadingModal('formunit');
					vm.$refs.FormUnit.hide(); 
					setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
				}
		...
		message: function (position, active) {
			if (position == 'error') {
				...
				else if (vm.position == 'pindahkamar') { vm.notification('Penambahan data gagal diproses.', 3000, position); }
				...
				else if (vm.position == 'pindahkamar') { vm.notification('Pembaharuan data berhasil diproses.', 3000, position); }
