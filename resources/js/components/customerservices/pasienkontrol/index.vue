...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/customerservices/pasienkontrol/list',
				add: '/customerservices/pasienkontrol/add',
				detail: '/customerservices/pasienkontrol/detail',
			}, url: '', data: null
		},
		column: [
			...
			{ value: 'sudah_mendaftar', label: 'Sudah Melakukan Pendaftaran', type: 'text', search: false, close: false, button: false },
			{ value: 'poli_tujuan', label: 'Poli Tujuan', type: 'text', search: false, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		btnhtml:function(_item, _index) {
			let str = [
				{ icon: 'arrow-up', color: 'btn-success', posisi: 'detail', tooltip: 'Daftarkan pasien ke Poli', item: _item, index: _index, show: true },
			]
			return str;
		},
		...
		politujuan:function(data) {
			if (data.poli_tujuan == 0) { return '-' }
			return 'Ruangan Poli ' + data.poli_tujuan;
		},
		...
		converter: function (data, index, column, identity) {
			...
			else if (identity == 'poli_tujuan') { _tmp = { value: vm.politujuan(data), ishtml: 'html', style: '' }; }
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				...
				vm.dialog('Yakin ingin menghapus data ' + data.jadwal_kontrol, 'Ya, hapus data', 'removedata');
			}
		},
