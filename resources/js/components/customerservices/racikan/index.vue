...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/customerservices/bebas/list',
				add: '/customerservices/bebas/add',
				detail: '/customerservices/bebas/detail',
				remove: '/customerservices/bebas/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nomor_antrian', label: 'No. Antrian', type: 'text', search: false, close: false, button: false },
			{ value: 'status_tindakan', label: 'Status Tindakan', type: 'text', search: false, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		nomorantrian:function(data) {
			let msg = "";
			if (data.number > 0 && data.number < 10) { msg = "00" + data.number }
			else if (data.number > 9 && data.number < 100) { msg = "0" + data.number }
			else if (data.number > 99 && data.number < 1000) { msg = "" + data.number }
			return data.kode + "-" + msg;
		},
		...
		converter: function (data, index, column, identity) {
			...
			else if (identity == 'nomor_antrian') { _tmp = { value: vm.nomorantrian(data), ishtml: 'html', style: '' }; }
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				...
				vm.$refs.FormUnit.show('detaildata', 'Detail Data', data.uuid, data);
				...
				vm.dialog('Yakin ingin menghapus data ' + data.nomor_antrian, 'Ya, hapus data', 'removedata');
			}
		},
		...
		parsingForm:function(data, key) {
			vm.attach.data = data;
			if (key == 'add') { vm.position = 'adddata'; vm.attach.url = vm.attach.link.add; } 
			else if (key == 'remove') { vm.position = 'removedata'; vm.attach.url = vm.attach.link.remove; } 
		},
