...
export default {
	...
	data:function() { return { 
		...
		tab: {
			...
			content: { histori: true, rawatjalan: false, pembelianobatkhusus: false }
		},
		position: '',
	}},
	methods: {
		...
		changesTab: function (values, index, classes) {
			...
			if (values == 'rawatjalan') {
				...
				if (vm.iskunjungan) { 
					setTimeout(() => {
						vm.$refs.FormRawatJalan.seteditedv2(vm.iskunjungan, vm.pj);
					}, 500);
		...
		*************************************************************************************************************************/
		...
		berhasil: function (response) {
			...
			else if (vm.position == 'editrawatjalan') { vm.$refs.FormRawatJalan.setedited(response);  }
