					...
					<div class="col-12">
						...
						<div class="tab-content">
							...
							<div class="content-tab-in" v-if="tab.content.histori">
								<HistoriRegistrasiInap ref="HistoriRegistrasiInap" :histori="histori"></HistoriRegistrasiInap>
							</div>

							<!-- Bagian tab content untuk data rawatinap -->
							<div class="content-tab-in" v-if="tab.content.rawatinap" >
								<FormRawatInap ref="FormRawatInap" @dialog="dialog" @cancel="cancel" @edit="edit" @parsingForm="parsingForm" :detail="detail" :iskunjungan="iskunjungan"></FormRawatInap>
...
export default {
	...
	components: { toast, Swal, 
		HistoriRegistrasiInap: defineAsyncComponent(() => import('./HistoriRegistrasiInap.vue')), 
		FormRawatInap: defineAsyncComponent(() => import('./FormRawatInap.vue')), 
	},
	...
	data:function() { return { 
		attach: {
			link : {
				...
				editrawatinap: '/customerservices/pasien/registrasi/editrawatinap',
				cancelrawatinap: '/customerservices/pasien/registrasi/cancelrawatinap',
				rawatinap: '/customerservices/pasien/registrasi/rawatinap',
		...
		tab: {
			...
			content: { histori: true, rawatinap: false, 
	...
	methods: {
		...
		changesTab: function (values, index, classes) {
			...
			if (values == 'rawatinap') {
				...
				if (vm.iskunjungan) { 
					setTimeout(() => {
						vm.$refs.FormRawatInap.seteditedv2(vm.iskunjungan, vm.pj, true);
					}, 500);
				}
				else {
					setTimeout(() => {
						vm.$refs.FormRawatInap.seteditedv3(false);
					}, 500);
				}
			}
		...
		edit:function(data, position) {
			if (position == 'rawatinap') {
				if (data) {
					vm.attach.data = new FormData();
					vm.attach.data.append('uuid', data.uuid);
					vm.attach.url = vm.attach.link.editrawatinap;
					vm.position = 'editrawatinap';
					vm.loaderprocess();
					vm.executions();
				}
			}
		...
		cancel:function(data, position) {
			if (position == 'rawatinap') {
				if (data) {
					vm.attach.data = new FormData();
					vm.attach.data.append('uuid', data.uuid);
					vm.attach.url = vm.attach.link.cancelrawatinap;
					vm.position = 'cancelrawatinap';
		...
		parsingForm:function(data, key) {
			vm.attach.data = data;
			if (key == 'rawatinap') { vm.attach.url = vm.attach.link.rawatinap; } 
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			vm.message('error', 1);
			vm.loaderprocess();
			if (vm.position == 'rawatinap') { }
			else if (vm.position == 'editrawatinap') { }
		...
		berhasil: function (response) {
			...
			else if (vm.position == 'editrawatinap') { vm.$refs.FormRawatInap.setedited(response, true);  }
			else if (vm.position == 'cancelrawatinap') { vm.$refs.FormRawatInap.resetform(); vm.hide(); vm.mainreload(); vm.iskunjungan = null; vm.pj = null;  }
			else if (vm.position == 'rawatinap') { }
			else if (vm.position == 'bedah') { }
		...
		message: function (position, active) {
			if (position == 'error') {
				if (vm.position == 'rawatinap') { vm.notification('Gagal memproses pasien rawat inap.', 3000, position); }
				else if (vm.position == 'editrawatinap') { vm.notification('Gagal memproses pasien rawat inap.', 3000, position); }
				else if (vm.position == 'cancelrawatinap') { vm.notification('Gagal memproses pasien rawat inap.', 3000, position); }
				...
				if (vm.position == 'rawatinap') { vm.notification('Berhasil memproses pasien rawat inap.', 3000, position); }
				else if (vm.position == 'cancelrawatinap') { vm.notification('Berhasil memproses pembatalan pasien rawat inap.', 3000, position); }
