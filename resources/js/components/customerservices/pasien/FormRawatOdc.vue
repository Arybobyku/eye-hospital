...
<div class="grid" style="border-top: 1px solid #d0d0d0; padding-top: 20px;" v-if="form">
	...
	<div class="col-4" style="text-align: right"  v-else>
		<button class="button-modal-page button-modal-red" v-on:click="cancel()">Batalkan One Day Care</button>
...
import { formrawatjalan } from './FormDataOdc.js';
import { parserawatjalan } from './AttachmentOdc.js';
...
export default {
    ...
    downloadImage() {
      ...
      console.log(canvas);
		...
		edit:function() {
			vm.$emit('edit', vm.test, 'rawatodc');
		},

		cancel:function() {
			vm.$emit('cancel', vm.test, 'rawatodc');
		},
		...
		parsingForm:function() { 
			vm.$emit('parsingForm', vm.parserawatjalan(vm.form, vm.detail), 'rawatodc'); 
		},
      ...
      vm.$emit('dialog', text, button, 'rawatodc');
    },
