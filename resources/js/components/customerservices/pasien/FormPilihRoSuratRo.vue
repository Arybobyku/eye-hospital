	...
	<div :style="terminate_detail.display" class="modal">
		<div ref="rootdetail" class="modal-content modal-besar" :class="terminate_detail.show ? 'modal-opened' : 'modal-closed'">
			<div class="modal-header">
				...
				<h2>Pilih RO Yang Dipakai Untuk Cetak Surat Pemeriksaan</h2>
			</div>
					...
					<table v-if="data" class="table">
						<thead>
							<tr>
								...
								<th>Tanggal</th>
								<th>Dokter</th>
								<th>Autorefkeratometry OD</th>
								<th>Autorefkeratometry OS</th>
								<th>Visus OD</th>
								<th>Visus OS</th>
								<th>Tonometry OD</th>
								<th>Tonometry OS</th>
						...
						<tbody>
							<tr v-for="row in data" :key="row.kode">
								<td>
									<a :href="`/customerservices/pasien/cetaksuratro/${row.uuid}`" download>
								...
								<td>{{ row.ocular_dextra_autoref }}</td>
								<td>{{ row.ocular_sinistra_autoref }}</td>
								<td>{{ row.ocular_dextra_visus }}</td>
								<td>{{ row.ocular_sinistra_visus }}</td>
								<td>{{ row.ocular_dextra_tonometri }}</td>
								<td>{{ row.ocular_sinistra_tonometri }}</td>
								<td></td>
								<td></td>
								<td></td>
								<td></td>
