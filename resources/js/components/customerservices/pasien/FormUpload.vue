	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-sedang" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			<div class="modal-header">
				<button v-on:click="action()">Upload File</button>
				<span class="close" v-on:click="hide()">&times;</span>
				<h2 v-if="form">Halaman Upload File</h2>
			</div>
...
import { formfile } from './FormData.js';
import { parsefile } from './Attachment.js';
var vm, body;
export default {
	...
	methods: {
		...
		aturulang: function () { vm.form = vm.formfile(); },
		...
		parsingForm:function() { 
			vm.$emit('parsingForm', 
			vm.parsefile(vm.form), 'uploadfile'); 
		},
		...
		dialog:function(){
			let text = '', button = '';
			text = 'Yakin ingin menambah data file ini.';
			button = 'Ya, tambah data';
      vm.$emit('dialog', text, button, 'uploadfile');
    },
	}
}
</script>
