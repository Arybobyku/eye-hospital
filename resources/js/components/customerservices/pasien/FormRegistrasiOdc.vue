					...
					<div class="col-12">
						...
						<div class="tab-content">
							...
							<div class="content-tab-in" v-if="tab.content.histori">
								<HistoriRegistrasiOdc ref="HistoriRegistrasiOdc" :histori="histori"></HistoriRegistrasiOdc>
							</div>

							<!-- Bagian tab content untuk data rawatodc -->
							<div class="content-tab-in" v-if="tab.content.rawatodc" >
								<FormRawatOdc ref="FormRawatOdc" @dialog="dialog" @cancel="cancel" @edit="edit" @parsingForm="parsingForm" :detail="detail" :iskunjungan="iskunjungan"></FormRawatOdc>
...
export default {
	...
	components: { toast, Swal, 
		HistoriRegistrasiOdc: defineAsyncComponent(() => import('./HistoriRegistrasiOdc.vue')), 
		FormRawatOdc: defineAsyncComponent(() => import('./FormRawatOdc.vue')), 
	},
	...
	data:function() { return { 
		attach: {
			link : {
				rawatodc: '/customerservices/pasien/registrasi/rawatodc',
				editrawatodc: '/customerservices/pasien/registrasi/editrawatodc',
				cancelrawatodc: '/customerservices/pasien/registrasi/cancelrawatodc',
				rawatodc: '/customerservices/pasien/registrasi/rawatodc',
		...
		tab: {
			button: [
				...
				{ value: 'rawatodc', label: 'One Day Care', class: 'tab-no-active' },
			...
			content: { histori: true, rawatodc: false, 
	...
	methods: {
		...
		changesTab: function (values, index, classes) {
			...
			if (values == 'rawatodc') {
				...
				if (vm.iskunjungan) { 
					setTimeout(() => {
						vm.$refs.FormRawatOdc.seteditedv2(vm.iskunjungan, vm.pj, true);
					}, 500);
				}
				else {
					setTimeout(() => {
						vm.$refs.FormRawatOdc.seteditedv3(false);
					}, 500);
				}
			}
		...
		edit:function(data, position) {
			if (position == 'rawatodc') {
				if (data) {
					vm.attach.data = new FormData();
					vm.attach.data.append('uuid', data.uuid);
					vm.attach.url = vm.attach.link.editrawatodc;
					vm.position = 'editrawatodc';
					vm.loaderprocess();
					vm.executions();
				}
			}
		...
		cancel:function(data, position) {
			if (position == 'rawatodc') {
				if (data) {
					vm.attach.data = new FormData();
					vm.attach.data.append('uuid', data.uuid);
					vm.attach.url = vm.attach.link.cancelrawatodc;
					vm.position = 'cancelrawatodc';
		...
		parsingForm:function(data, key) {
			vm.attach.data = data;
			if (key == 'rawatodc') { vm.attach.url = vm.attach.link.rawatodc; } 
			else if (key == 'rawatodc') { vm.attach.url = vm.attach.link.rawatodc; } 
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			vm.message('error', 1);
			vm.loaderprocess();
			if (vm.position == 'rawatodc') { }
			else if (vm.position == 'editrawatodc') { }
			else if (vm.position == 'rawatodc') { }
		...
		berhasil: function (response) {
			...
			else if (vm.position == 'editrawatodc') { vm.$refs.FormRawatOdc.setedited(response, true);  }
			else if (vm.position == 'cancelrawatodc') { vm.$refs.FormRawatOdc.resetform(); vm.hide(); vm.mainreload(); vm.iskunjungan = null; vm.pj = null;  }
			else if (vm.position == 'rawatodc') { }
			else if (vm.position == 'bedah') { }
		...
		message: function (position, active) {
			if (position == 'error') {
				if (vm.position == 'rawatodc') { vm.notification('Gagal memproses pasien rawat odc.', 3000, position); }
				else if (vm.position == 'editrawatodc') { vm.notification('Gagal memproses pasien rawat odc.', 3000, position); }
				else if (vm.position == 'cancelrawatodc') { vm.notification('Gagal memproses pasien rawat odc.', 3000, position); }
				else if (vm.position == 'rawatodc') { vm.notification('Gagal memproses pasien rawat odc.', 3000, position); }
				...
				if (vm.position == 'rawatodc') { vm.notification('Berhasil memproses pasien rawat odc.', 3000, position); }
				else if (vm.position == 'cancelrawatodc') { vm.notification('Berhasil memproses pembatalan pasien rawat odc.', 3000, position); }
				else if (vm.position == 'rawatodc') { vm.notification('Berhasil memproses pasien rawat odc.', 3000, position); }
