...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/customerservices/pasienactive/list',
				listbelum: '/customerservices/pasienactive/listbelum',
				listsudah: '/customerservices/pasienactive/listsudah',
				detail: '/customerservices/pasienactive/detail',
			}, url: '', data: null
		},
		tab: {
			button: [
		...
		berhasil: function (response) {
			...
			if (response.data.data == 'cannot') {
				...
				if (vm.position == 'loadmain') { 
					...
					if (vm.posisieksternal=='pasienbelum') {
						vm.$refs.DatatableBelum.update('', vm.setDatatablebelum(response.data.data, response.data.total), response.data.total); 
						vm.$refs.DatatableBelum.skeleton(); 
						vm.$refs.DatatableBelum.paging(); 
						active = 0;
					}
					else if (vm.posisieksternal=='pasiensudah') {
						vm.$refs.DatatableSudah.update('', vm.setDatatablesudah(response.data.data, response.data.total), response.data.total); 
						vm.$refs.DatatableSudah.skeleton(); 
						vm.$refs.DatatableSudah.paging(); 
						active = 0;
					}
