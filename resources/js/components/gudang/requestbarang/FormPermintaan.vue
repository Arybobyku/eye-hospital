	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-besar" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			<div class="modal-header">
				<button v-on:click="action()">Save Item Pesanan</button>
				<span class="close" v-on:click="hide()">&times;</span>
				<h2 v-if="form">Data Pemesanan Barang</h2>
			</div>
						...
						<table class="table">
							...
							<tbody v-if="listobat.length > 0">
								<tr v-for="(item, index) in listobat">
									...
									<td>{{ item.jenis }}</td>
									<td>{{ item.nama }}</td>
									<td>{{ item.jumlah_permintaan_kecil }}</td>
...
export default {
	...
	methods: {
		...
		formpermintaan, parsepermintaan, formatrupiah,
		...
		htgtotals:function(item) {
			return item.harga_netto * item.jumlah_permintaan_kecil;
		},
		...
		additemobat:function() {
			...
			if (vm.tempobat) {
				let str = {
					...
					jumlah_permintaan_besar: vm.form.jumlah.value,
					jumlah_permintaan_kecil: vm.form.jumlah.value,
					labeling: '',
					keterangan: vm.form.keterangan.value,
				}
				vm.listobat.push(str);
				vm.form.select.hargagudang.value = '';
				vm.form.select.hargagudang.label = 'Silahkan Pilih';
				vm.form.jumlah.value = '';
				vm.form.keterangan.value = '';
			}
		},
		...
		show:function(posisi, title, uuid){ 
			...
			vm.form.uuid = uuid;
			vm.form.label_permintaan_uuid = uuid;
		...
		aturulang: function () { vm.form = vm.formpermintaan(); },
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parsepermintaan(vm.form, vm.listobat), 'permintaan'); },
			...
			if (tmp.length > 0) {
				for (let i = 0; i < tmp.length; i++) {
					let str = {
						...
						jumlah_permintaan_besar: tmp[i].jumlah_permintaan_besar,
						jumlah_permintaan_kecil: tmp[i].jumlah_permintaan_kecil,
