...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/gudang/obat/list',
				add: '/gudang/obat/add',
				edit: '/gudang/obat/edit',
				update: '/gudang/obat/update',
				remove: '/gudang/obat/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama Obat', type: 'text', search: true, close: false, button: false },
			{ value: 'nama_satuan_besar', label: 'Satuan Terbesar', type: 'text', search: true, close: false, button: false },
			{ value: 'nama_satuan_kecil', label: 'Satuan Terkecil', type: 'text', search: true, close: false, button: false },
			...
			{ value: 'min_stock', label: 'Min Stock', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		kalkulasi:function (_item) {
			return _item.hitung_besar + ' ' + _item.nama_satuan_besar + ' <strong>=</strong> ' + _item.hitung_kecil +  ' ' + _item.nama_satuan_kecil;
		},
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				...
				vm.position = "adddata";
				vm.$refs.FormObat.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormObat.aturulang();
				vm.position = "editdata";
				vm.$refs.FormObat.show('editdata', 'Edit Data', data.uuid);
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formobat'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formobat'); vm.$refs.FormObat.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formobat'); }
		...
		runconfirm: function (posisi) {
			if (posisi == 'formobat') { vm.loadingModal('formobat'); }
