...
<FormSatuan ref="FormSatuan" @dialog="dialog" @parsingForm="parsingForm"></FormSatuan>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormSatuan: defineAsyncComponent(() => import('./FormSatuan.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/gudang/satuan/list',
				add: '/gudang/satuan/add',
				edit: '/gudang/satuan/edit',
				update: '/gudang/satuan/update',
				remove: '/gudang/satuan/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama Satuan', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormSatuan.aturulang();
				vm.position = "adddata";
				vm.$refs.FormSatuan.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormSatuan.aturulang();
				vm.position = "editdata";
				vm.$refs.FormSatuan.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formsatuan'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formsatuan') { vm.$refs.FormSatuan.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formsatuan'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formsatuan'); vm.$refs.FormSatuan.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formsatuan'); }
		...
		runconfirm: function (posisi) {
			if (posisi == 'formsatuan') { vm.loadingModal('formsatuan'); }
