			...
			<div class="modal-body" v-if="form">
				<div class="grid">
					<div class="col-12">
						...
						<Inputed :ref="form.kontak.name" :form="form.kontak"></Inputed>
					</div>
				</div>
			</div>
...
import { formsupplier } from './FormData.js';
import { parsesupplier } from './Attachment.js';
var vm, body;
export default {
	...
	methods: {
		...
		aturulang: function () { vm.form = vm.formsupplier(); },
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parsesupplier(vm.form), 'supplier'); },
		...
		setdataform: function (response) {
			...
			vm.form.alamat.value = response.data.data.alamat;
			vm.form.kontak.value = response.data.data.kontak;
			vm.loaderprocess();
		},
      ...
      vm.$emit('dialog', text, button, 'formsupplier');
    },
	}
}
</script>