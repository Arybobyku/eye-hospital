...
<FormSupplier ref="FormSupplier" @dialog="dialog" @parsingForm="parsingForm"></FormSupplier>
</template>
...
export default {
	...
	components: { toast, Swal, 
		FormSupplier: defineAsyncComponent(() => import('./FormSupplier.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/gudang/supplier/list',
				add: '/gudang/supplier/add',
				edit: '/gudang/supplier/edit',
				update: '/gudang/supplier/update',
				remove: '/gudang/supplier/remove',
			}, url: '', data: null
		},
		column: [
			{ value: 'nama', label: 'Nama Supplier', type: 'text', search: true, close: false, button: false },
			{ value: 'alamat', label: 'Alamat', type: 'text', search: true, close: false, button: false },
			{ value: 'kontak', label: 'Kontak', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				vm.$refs.FormSupplier.aturulang();
				vm.position = "adddata";
				vm.$refs.FormSupplier.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormSupplier.aturulang();
				vm.position = "editdata";
				vm.$refs.FormSupplier.show('editdata', 'Edit Data', data.uuid);
				setTimeout(() => { vm.loadingModal('formsupplier'); }, 250, this);
		...
		loadingModal: function (position) { 
			if (position == 'formsupplier') { vm.$refs.FormSupplier.loaderprocess();  }
		},
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'adddata') { vm.loadingModal('formsupplier'); }
			else if (vm.position == 'editdata') { vm.loadingModal('formsupplier'); vm.$refs.FormSupplier.hide();  }
			else if (vm.position == 'updatedata') { vm.loadingModal('formsupplier'); }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
				...
				vm.loadingModal('formsupplier');
				vm.$refs.FormSupplier.hide(); 
				setTimeout(() => { vm.$refs.Datatable.skeleton(); vm.tablereload(); }, 500, this);
			}
			else if (vm.position == 'editdata') {
				vm.$refs.FormSupplier.setdataform(response); 
		...
		runconfirm: function (posisi) {
			if (posisi == 'formsupplier') { vm.loadingModal('formsupplier'); }
