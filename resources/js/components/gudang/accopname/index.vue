...
<FormPermintaan ref="FormPermintaan"></FormPermintaan>
</template>
...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/gudang/accopname/list',
				detail: '/gudang/accopname/detail',
				proses: '/gudang/accopname/proses',
				kirim: '/gudang/accopname/kirim',
				tolak: '/gudang/accopname/tolak',
			}, url: '', data: null
		},
		column: [
			...
			{ value: 'dari_nama_unit', label: 'Dari Unit', type: 'text', search: false, close: false, button: false },
		...
		*************************************************************************************************************************/
		...
		btnhtml:function(_item, _index) {
			let str = [
				...
				{ 
					icon: 'upload-cloud', color: 'btn-warning', posisi: 'proses', tooltip: 'Proses Permintaan', 
					item: _item, index: _index, show: _item.status == 'Permintaan' ? true : false },
				{ 
					icon: 'send', color: 'btn-warning', posisi: 'kirim', tooltip: 'Kirim Obat/Alkes', 
					item: _item, index: _index, show: _item.status == 'Diproses' ? true : false },
			]
			return str;
		},
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				...
				vm.dialog('Yakin ingin memproses permintaan obat/alkes ini.', 'Ya, proses permintaan', 'prosesdata');
			}
			else if (posisi == 'kirim') {
				vm.position = "kirimdata";
				vm.attach.data = new FormData();
				vm.attach.data.append('kode', data.kode);
				vm.attach.url = vm.attach.link.kirim;
				vm.dialog('Yakin ingin mengirim obat/alkes ke unit yang meminta.', 'Ya, kirim obat/alkes', 'kirimdata');
			}
		},
		...
		*************************************************************************************************************************/
		...
		message: function (position, active) {
			if (position == 'error') {
				...
				else if (vm.position == 'prosesdata') { vm.notification('Permintaan obat/alkes gagal diproses.', 3000, position); }
				else if (vm.position == 'kirimdata') { vm.notification('Pengiriman obat/alkes gagal diproses.', 3000, position); }
			}
				...
				else if (vm.position == 'prosesdata') { vm.notification('Permintaan obat/alkes berhasil diproses.', 3000, position); }
				else if (vm.position == 'kirimdata') { vm.notification('Pengiriman obat/alkes berhasil diproses.', 3000, position); }
			}
		},
