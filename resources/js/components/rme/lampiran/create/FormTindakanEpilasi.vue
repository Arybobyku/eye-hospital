        ...
        <div class="signature-section-single">
          <div class="sign-box-center">
            ...
            <div v-if="form.ttd_dpjp && !signatureCleared" class="signature-preview">
              ...
              <p v-if="form.dpjp_ttd_timestamp" class="timestamp-ttd">
                Ditandatangani: {{ form.dpjp_ttd_timestamp }}
              </p>
...
export default {
  ...
  methods: {
    ...
    async fetchTahunAkreditasi() {
      try {
        ...
        if (!this.form.no_surat) {
          this.form.no_surat = `RM 10.4/LE/${tahun}`;
        }
        ...
        if (!this.form.no_surat) {
          this.form.no_surat = 'RM 10.4/LE/22';
        }
      }
    },
    setDataForm() {
      ...
      try {
        ...
        if (typeof this.editData === "string") {
          const response = await axios.get(
            `/master/pasien/dokumen-tindakan-epilasi/${this.editData}`
          );
    ...
    clearSignature() {
      ...
      this.form.ttd_dpjp = "";
      this.form.dpjp_ttd_timestamp = "";


      // Reset signature pad di next tick
      this.$nextTick(() => {
        const pad = this.$refs.ttd_dpjp;
        if (pad) {
          pad.clearSignature();
        }
      });
    ...
    saveSign() {
      ...
      if (!pad) {
        console.error("REF tidak ditemukan: ttd_dpjp");
        return;
      }
      ...
      this.form.ttd_dpjp = data;
      this.signatureCleared = false;

      const now = new Date();
      this.form.dpjp_ttd_timestamp = now.toLocaleString('id-ID', {
        day: '2-digit', month: '2-digit', year: 'numeric',
        hour: '2-digit', minute: '2-digit', second: '2-digit'
      });
      console.log("TTD saved: ttd_dpjp");
    },
    ...
    async submitForm() {
      ...
      try {
        ...
        const message = this.isEditMode
          ? "Form Tindakan Epilasi berhasil diupdate!"
          : "Form Tindakan Epilasi berhasil disimpan!";
