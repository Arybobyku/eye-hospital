...
<div class="grid" style="border-top: 1px solid #d0d0d0; padding-top: 20px;" v-if="form">
	...
	<div class="col-4" style="text-align: right"  v-else>
		<button class="button-modal-page button-modal-red" v-on:click="cancel()">Batalkan Penanganan</button>
...
import { formigd } from './FormData.js';
import { parseigd } from './Attachment.js';
...
export default {
	...
	methods: {
		...
		formigd, parseigd, arrregistrasi,
		...
		redbutton:function() {
			if (vm.red == 'Clear Form') { vm.form = vm.formigd(); }
		...
		seteditedv2:function(response, pj) {
			...
			vm.form.select.dokterumum.value =  response.pengguna_umum_uuid;
		...
		setedited:function(response) {
			...
			vm.form.select.dokterumum.value =  response.data.data.pengguna_umum_uuid;
			vm.form.select.dokterumum.label =  response.data.data.nama_dokter_umum;
		...
		edit:function() {
			vm.$emit('edit', vm.test, 'igd');
		},

		cancel:function() {
			vm.$emit('cancel', vm.test, 'igd');
		},
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parseigd(vm.form, vm.detail), 'igd'); },

		dialog:function(){
			let text = '', button = '';
			text = 'Yakin ingin menambah data pada halaman ini.';
			button = 'Ya, tambah data';
      vm.$emit('dialog', text, button, 'igd');
    },
