		...
		<div ref="rootmodal" class="modal-content modal-sedang" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			...
			<div class="modal-body" v-if="form">
				<div class="grid">
					...
					<div class="col-12">
						<Inputed :ref="form.nofaktur.name" :form="form.nofaktur"></Inputed>
						<Inputed :ref="form.tanggalfaktur.name" :form="form.tanggalfaktur"></Inputed>
						<Inputed :ref="form.ppn.name" :form="form.ppn"></Inputed>

						<Selected v-on:click="selectbox($event, form.select.pembayaran.name, form.select.pembayaran.statics)" 
							:ref="form.select.pembayaran.name" @selecteditem="selecteditem" @selectclear="selectclear"
							:selection="form.select.pembayaran"></Selected>

						<Inputed :ref="form.jangkawaktu.name" :form="form.jangkawaktu" v-if="form.select.pembayaran.value == 'Kredit'"></Inputed>
...
import { formfaktur } from './FormData.js';
import { parsefaktur } from './Attachment.js';
...
export default {
	...
	mounted:function() { 
		...
		vm.arr = vm.arrpembelian();
		vm.form = vm.formfaktur();
	...
	methods: {
		...
		parsefaktur, formfaktur, initindexdb, indexdbprocessing, arrpembelian,
		...
		selecteditem:function(item, key) { 
			...
			if (key == 'pembayaran') {
				if (vm.form.select.pembayaran.value == 'Kredit') {
					vm.form.jangkawaktu.required = 'required';
				}
				else {
					vm.form.jangkawaktu.required = '';
				}
			}
		},
		selectclear:function(key) { 
			if (key == 'pembayaran') { vm.form.jangkawaktu.required = ''; }
		...
		aturulang: function () { vm.form = vm.formfaktur(); },
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parsefaktur(vm.form), 'faktur'); },
		...
		setdataform: function (response) {
			...
			vm.form.nofaktur.value = data.no_faktur;
			vm.form.select.supplier.value = data.supplier_uuid;
			vm.form.select.supplier.label = data.nama_supplier;
			vm.form.tanggalfaktur.value = data.tanggal_faktur;
			vm.form.keterangan.value = data.keterangan;
			vm.form.ppn.value = data.ppn;
			vm.form.select.pembayaran.value = data.pembayaran;
			vm.form.select.pembayaran.label = data.pembayaran;
			if (vm.form.select.pembayaran.value == 'Kredit') {
				vm.form.jangkawaktu.required = 'required';
			}
			else {
			...
			vm.form.jangkawaktu.value = data.jangka_waktu;
      ...
      vm.$emit('dialog', text, button, 'formfaktur');
    },
	}
}
</script>