...
<FormKwitansi ref="FormKwitansi" @dialog="dialog" @parsingForm="parsingForm"></FormKwitansi>
</template>
...
export default {
	...
	components: { toast, Swal, 
		...
		FormKwitansi: defineAsyncComponent(() => import('./FormKwitansi.vue')),
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/finance/claim/list',
				detail: '/finance/claim/detail',
				claim: '/print/claim/',
				kwitansiclaim: '/finance/kwitansiclaim/get',
				addkwitansiclaim: '/finance/kwitansiclaim/add',
				diterimakwitansiclaim: '/finance/kwitansiclaim/diterima',
				pengantar: '/print/pengantar/',
			}, url: '', data: null
		},
		column: [
			{ value: 'tanggal', label: 'Tgl Pendaftaran', type: 'date', search: true, close: false, button: false },
			{ value: 'tanggal_bayar', label: 'Tgl Bayar', type: 'date', search: true, close: false, button: false },
			...
			{ value: 'kwitansi_claim', label: 'No Kwitansi (Claim)', type: 'text', search: true, close: false, button: false },
			{ value: 'diskon_claim', label: 'Diskon ke Asuransi', type: 'text', search: true, close: false, button: false },
			{ value: 'status_claim', label: 'Status Pembayaran Claim', type: 'text', search: true, close: false, button: false },
		...
		tab: {
			button: [
				{ value: 'today', label: 'List Tagihan', class: 'tab-active' },
				// { value: 'bayar', label: 'Tagihan (Sudah Bayar)', class: 'tab-no-active' },
				// { value: 'beli', label: 'Pasien Bebas', class: 'tab-no-active' },
			],
			content: { 
				today: true, 
				// bayar: false, 
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		btnhtml:function(_item, _index) {
			let str = [
				...
				{ icon: 'printer', color: 'btn-info', posisi: 'pengantar', tooltip: 'Cetak Surat Pengantar', item: _item, index: _index, 
				show: _item.status_kasir == 'Sudah Bayar' ? true : false },
				{ icon: 'printer', color: 'btn-info', posisi: 'kwitansi', tooltip: 'Input No Kwitansi', item: _item, index: _index, 
				show: _item.status_kasir == 'Sudah Bayar' && _item.status_claim == 'Belum Diterima' ? true : false },
				{ icon: 'printer', color: 'btn-info', posisi: 'diterima', tooltip: 'Ubah Status Menjadi Diterima', item: _item, index: _index, 
				show: _item.status_kasir == 'Sudah Bayar' && _item.status_claim == 'Belum Diterima' ? true : false },
		...
		diskonclaim:function(data) {
			if (data.diskon_claim != '0' && data.diskon_claim != 0) { return data.diskon_claim + '%'; }
			return '-';
		},
		...
		converter: function (data, index, column, identity) {
			...
			else if (identity == 'diskon_claim') { _tmp = { value: vm.diskonclaim(data), ishtml: 'html', style: '' }; }
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'kwitansi') {
				vm.$refs.FormKwitansi.aturulang();
				vm.position = "kwitansidata";
				vm.$refs.FormKwitansi.show('kwitansidata', 'Input No. Kwitansi', data.uuid);
				setTimeout(() => { vm.loadingModal('formkwitansi'); }, 250, this);
				vm.attach.data = new FormData();
				vm.attach.data.append('registrasi_uuid', data.uuid);
				vm.attach.url = vm.attach.link.kwitansiclaim;
				vm.executions();
			}
				...
				window.open(vm.attach.link.claim + data.uuid, '_blank');
			}
			else if (posisi == 'pengantar') {
				window.open(vm.attach.link.pengantar + data.uuid, '_blank');
			}
			...
			else if (posisi == 'diterima') {
				vm.position = "terimadata";
				vm.attach.data = new FormData();
				vm.attach.data.append('registrasi_uuid', data.uuid);
				vm.attach.url = vm.attach.link.diterimakwitansiclaim;
				vm.dialog('Yakin ingin merubah status pembayaran menjadi sudah diterima.', 'Ya, Ubah Status', 'diterima');
			}
		},

		loadingModal: function (position) { 
			if (position == 'formkwitansi') { vm.$refs.FormKwitansi.loaderprocess();  }
		...
		parsingForm:function(data, key) {
			vm.attach.data = data;
			if (key == 'add') { vm.position = 'updatedata'; vm.attach.url = vm.attach.link.addkwitansiclaim; }
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'kwitansidata') { vm.loadingModal('formkwitansi'); vm.$refs.FormKwitansi.hide();  }
		...
		berhasil: function (response) {
			...
			if (vm.position == 'loadmain') { 
			...
			else if (vm.position == 'kwitansidata') {
				vm.$refs.FormKwitansi.setdataform(response); 
				...
				vm.loadingModal('formkwitansi');
				vm.$refs.FormKwitansi.hide(); 
		...
		message: function (position, active) {
			if (position == 'error') {
				...
				else if (vm.position == 'kwitansidata') { vm.notification('Proses pengambilan data gagal dilakukan.', 3000, position); }
				...
				else if (vm.position == 'terimadata') { vm.notification('Perubahan status gagal diproses.', 3000, position); }
				...
				if (vm.position == 'updatedata') { vm.notification('Nomor Kwitansi claim berhasil ditambahkan/diperbaharui.', 3000, position); }
				...
				else if (vm.position == 'terimadata') { vm.notification('Perubahan status berhasil diproses.', 3000, position); }
		...
		runconfirm: function (posisi) {
			...
			else if (posisi == 'formkwitansi') { vm.loadingModal('formkwitansi'); }
			else if (posisi == 'diterima') { vm.$refs.Datatable.skeleton(); }
			vm.executions();
		},
