...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/finance/historibebas/list',
		...
		tab: {
			button: [
				{ value: 'today', label: 'Pasien Bebas (Aktif)', class: 'tab-active' },
			],
			content: { 
				today: true, 
				bayar: false, 
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		converter: function (data, index, column, identity) {
			...
			else if (identity == 'tanggal') { _tmp = { value: vm.nopendaftaran(data), ishtml: 'html', style: '' }; }
