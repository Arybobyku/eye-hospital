...
<div class="inner" ref="roottable">
	...
	<div class="tab-content">
		...
		<div class="content-tab-in" v-if="tab.content.bayar">
		...
		<div class="content-tab-in" v-if="tab.content.beli">
			<Datatable ref="DatatableBeli" :module="module" @tablereload="tablereload" @tablebutton="tablebutton"></Datatable>
		</div>
	</div>
...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/finance/bedahkasir/list',
				listbayar: '/finance/bedahkasir/listbayar',
				listtriase: '/finance/bedahkasir/panjar',
				call: '/finance/bedahkasir/call',
				detail: '/finance/bedahkasir/detail',
				bayar: '/finance/bedahkasir/bayar',
				cancelbayar: '/finance/kasir/cancelbayar',
				terima: '/finance/bedahkasir/terima',
				kasir: '/print/kasirinap/',
				kasirrincian: '/print/kasirrincianinap/',
				panjar: '/print/panjar/',
				getpanjar: '/finance/bedahkasir/getpanjar',
				addpanjar: '/finance/bedahkasir/addpanjar',
				hapusbiaya: '/finance/bedahkasir/hapusbiaya',
				perbaharuibiaya: '/finance/bedahkasir/perbaharuibiaya',
		...
		column: [
			{ value: 'jenis', label: 'Layanan', type: 'text', search: true, close: false, button: false },
		...
		columntriase: [
			...
			{ value: 'status_kasir', label: 'Status Pelunasan', type: 'text', search: false, close: false, button: false },
			{ value: 'approvement_obat', label: 'Obat-Obatan', type: 'text', search: false, close: false, button: false },
		...
		columnbeli: [
			...
			{ value: 'number', label: 'Nomor Pendaftaran', type: 'text', search: true, close: false, button: false },
		...
		tab: {
			button: [
				...
				// { value: 'beli', label: 'Pasien membeli obat', class: 'tab-no-active' },
			],
			content: { 
				today: true, 
				bayar: false, 
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		btnhtmltriase:function(_item, _index) {
			let str = [
				{ icon: 'dollar-sign', color: 'btn-success', posisi: 'terima', tooltip: 'Terima Panjar', item: _item, index: _index, 
				show: _item.approve_panjar == '0' ? true : false },
				{ icon: 'bell', color: 'btn-info', posisi: 'panggil1', tooltip: 'Panggil Pasien', item: _item, index: _index, 
				show: _item.status_kasir == 'Belum Bayar' ? true : false },
				{ icon: 'arrow-up', color: 'btn-success', posisi: 'detail', tooltip: 'Detail Data', item: _item, index: _index, 
				show: _item.approve_panjar == '1' ? true : false },
				{ icon: 'printer', color: 'btn-info', posisi: 'printpanjar', tooltip: 'Cetak Bukti Panjar', item: _item, index: _index, 
				show: _item.status_kasir == 'Belum Bayar' ? true : false },
				{ icon: 'printer', color: 'btn-info', posisi: 'print', tooltip: 'Cetak Tagihan', item: _item, index: _index, 
				show: _item.status_kasir == 'Sudah Bayar' ? true : false },
		...
		convertertriase: function (data, index, column, identity) {
			...
			else if (identity == 'status_kasir') { _tmp = { value: vm.statuspelunasan(data), ishtml: 'html', style: '' }; }
			else { _tmp = { value: column, ishtml: 'text', style: '' } }
			return _tmp != '' ? _tmp : 'empty';
		},
		...
		tablereload:function(data = new FormData(), pos = 'main') { 
			...
			else if (vm.posisieksternal == 'bayar') {
				if (pos == 'outer') {
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'call1') { vm.$refs.DatatableTriase.skeleton(); }
			...
			else if (vm.position == 'terimadata') { vm.$refs.DatatableTriase.skeleton(); }
		...
		runconfirm: function (posisi) {
			...
			else if (posisi == 'terimadata') { vm.$refs.DatatableTriase.skeleton(); }
			else if (posisi == 'call') { vm.$refs.Datatable.skeleton(); }
			else if (posisi == 'call1') { vm.$refs.DatatableTriase.skeleton(); }
