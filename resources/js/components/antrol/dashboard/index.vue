        ...
        methods: {
			...
			async applyFilter() {
				...
				if (this.filterType === "tanggal") {
					...
					url = `/antrol-bpjs/dashboard/waktutunggu/tanggal/${this.params1}?ts=${Date.now()}`;
				} else if (this.filterType === "bulan") {
					if (!this.params1 || !this.params2) {
						alert("Silakan pilih bulan dan tahun");
						return;
					}
					url = `/antrol-bpjs/dashboard/waktutunggu/bulan/${this.params1}/tahun/${this.params2}?ts=${Date.now()}`;
				}
				...
				try {
					...
					const processedData = this.setDatatable(response.data.data, response.data.data.length);
					this.module.data = processedData;
					this.module.total = response.data.data.length;

					this.$refs.Datatable.update(this.column, processedData, response.data.data.length);
