...
<template>
    <div class="filter-container">
        <input type="text" v-model="params1" />

        <button @click="applyFilter">Cari</button>
    </div>
...
export default {
    ...
    methods: {
         ...
         *************************************************************************************************************************/
        async applyFilter() {
            let url = `/api/bpjs/antrol-bpjs/antrean/getlisttask/${this.params1}`;
            ...
            try {
                ...
                const processedData = this.setDatatable(
                    response.data.response,
        ...
        tablebutton: function (posisi, data, index) {
            if (posisi == "add") {
                ...
                vm.dialog(
                    "Yakin ingin menghapus data yang terpilih dihalaman ini.",
                    "Ya, hapus data",
                    "blockdata"
                );
            }
        ...
        message: function (position, active) {
            if (position == "error") {
                if (vm.position == "loadmain") {
                    ...
                    vm.notification(
                        "Penghapusan data gagal diproses.",
                        3000,
                        position
                    );
                } else if (vm.position == "activedata") {
                    vm.notification(
                        "Pengaktifan data gagal diproses.",
                        3000,
                        position
                    );
                }
                ...
                if (vm.position == "adddata") {
                    ...
                    vm.notification(
                        "Penghapusan data berhasil diproses.",
                        3000,
                        position
                    );
                } else if (vm.position == "activedata") {
                    vm.notification(
                        "Pengaktifan data berhasil diproses.",
                        3000,
                        position
                    );
                }
