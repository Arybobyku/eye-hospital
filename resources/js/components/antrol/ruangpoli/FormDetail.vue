	...
	<div :style="terminate.display" class="modal">
		<div ref="rootmodal" class="modal-content modal-semi-besar" :class="terminate.show ? 'modal-opened' : 'modal-closed'">
			<div class="modal-header">
				...
				<h2 v-if="form">Ruang Poli</h2>
			</div>
			<div class="modal-body" v-if="form">
				<div class="grid">
					<div class="col-12">
						<Inputed :ref="form.ruang_poli.name" :form="form.ruang_poli"></Inputed>
					</div>
					<div class="col-12">
...
export default {
	...
	methods: {
		...
		parsedetail, formdetail, initindexdb, indexdbprocessing,
		...
		parsingForm:function() { vm.$emit('parsingForm', vm.parsedetail(vm.form), 'detail'); },
		...
		setdataform: function (response) {
			...
			vm.form.ruang_poli.value = response.data.data.ruang_poli;
			if (response.data.data) {
				if (response.data.data.nama_dokter != '-' && response.data.nama_dokter != '') {
					vm.form.select.dokter.value = response.data.data.dokter_uuid;
