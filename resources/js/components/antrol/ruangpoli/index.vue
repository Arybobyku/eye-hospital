...
export default {
	...
	data: function () { return {
		...
		attach: {
			link : {
				list: '/dokter/bpjs/list',
				edit: '/dokter/bpjs/detail',
				update: '/dokter/bpjs/update',
			}, url: '', data: null
		},
		column: [
			{ value: 'ruang_poli', label: 'Ruang Poli', type: 'text', search: true, close: false, button: false },
			{ value: 'kode_dokter_bpjs', label: 'Kode Dokter BPJS', type: 'text', search: true, close: false, button: false },
	...
	methods: {
		...
		*************************************************************************************************************************/
		...
		tablebutton:function(posisi, data, index) {
			if (posisi == 'add') {
				...
				vm.position = "adddata";
				vm.$refs.FormDetail.show('adddata', 'Tambah Data', '');
			}
			else if (posisi == 'edit') {
				vm.$refs.FormDetail.aturulang();
		...
		*************************************************************************************************************************/
		...
		gagal: function (error) {
			...
			else if (vm.position == 'editdata') { vm.loadingModal('formdetail'); vm.$refs.FormDetail.hide();  }
