			...
			<td style="width: 58%">
				<table class="header-top">
					<tr>
						...
						<td>: NP{{ $registrasi->no_invoice }}</td>
					</tr>
					<tr>
			...
			<td valign="top">
				<table class="header-top">
					...
					<tr>
						...
						<td>: {{ tglse($registrasi->tanggal_selesai_periksa) }} {{ $registrasi->dokter_jam_selesai }}</td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tbody>
				...
				@foreach ($administrasi as $item)
					...
					<tr>
						...
						<td align="left" style="padding: 10px 2px;">
							...
							@elseif($item->nama_layanan == 'Administrasi Pelayanan Rawat Inap')
								Administrasi Pelayanan Rawat Inap
							@endif
							
						</td>
						<td align="center" style="padding: 10px 2px; width: 17%" valign="top">{{ ubahTanpaJam($item->created_at) }}</td>
			...
			@if(count($rawatjalan) > 0)
				...
				@foreach ($rawatjalan as $item)
					...
					<tr>
						<td colspan="8" style="padding: 13px 8px 11px"><b>Tindakan Medik Rawat Inap</b> : <b>{{ $item->nama_dokter }}</b></td>
					</tr>
			...
			@if(count($honor) > 0)
				<tr>
					<td colspan="8" style="padding: 13px 8px 11px"><b>Honor Visite Dokter</b></td>
				</tr>
			...
			@if($collection->count() > 0)
				...
				@foreach ($collection as $item)
					...
					@foreach ($item->data as $row)
							@if ($nomor == 1) 
								<tr>
									<td colspan="8" style="padding: 13px 8px 11px"><b>
										@if ($row->jenis == 'Kamar')
											Tarif Kamar Rawat Inap
										@elseif ($row->jenis == 'Registrasi (sos)' || $row->jenis == 'Registrasi' || $row->jenis == 'Administrasi')
											Biaya Administrasi
										@else
											{{ $row->jenis }}
										@endif
							...
							<tr>
								...
								<td align="center" style="padding: 10px 2px; width: 17%" valign="top">{{ ubahTanpaJam($row->created_at) }}</td>
			...
			@if(count($room) > 0 && $collection->count() < 1)
				...
				@foreach ($room as $item)
					...
					<tr>
						...
						<td align="center" style="padding: 10px 3px; width: 17%" valign="top">{{ ubahTanpaJam($item->created_at) }}</td>
			...
			@if(count($resepracikan) > 0)
				...
				@foreach ($resepracikan as $item)
					<tr>
						...
						<td align="center" style="padding: 10px 2px; width: 17%" colspan="2" valign="top">{{ ubahTanpaJam($item->created_at) }}</td>
			...
			@if (
				...
				@elseif (($registrasi->apakah_paket == 'Tidak' || $registrasi->apakah_paket == 'tidak') && $registrasi->cover_asuransi != 0)
					<tr>
						<td colspan="6" colspan="2" align="left" style="padding: 4px 7px; width: 65%"><b>Dibayarkan pasien</b></td>
						<td colspan="2" align="right" style="padding: 4px 7px"><b>Rp. {{ number_format($grandtotal) }}</b></td>
					</tr>
				@endif
			@endif
			...
			<tr>
				<td colspan="8" style="font-size: 10pt">*) Harga obat dan alat kesehatan diatas sudah termasuk PPN 11%</td>
			</tr>
		</tbody>
	</table>
