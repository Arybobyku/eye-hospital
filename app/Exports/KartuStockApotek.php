...
class KartuStockApotek implements FromView, ShouldAutoSize
{
  ...
  public function view(): View
  {
		...
		->select(
			...
			DB::raw("minta_kecil as masuk_kecil"),
			'jumlah_kecil',
	...
	$resepQuery = DB::table('resep')
		->select(
			DB::raw("registrasi.no_kwitansi as kode"),
			DB::raw("nama_obat as nama"),
			DB::raw("'Penjualan' as ke_nama_unit"),
			DB::raw("'Apotek' as dari_nama_unit"),
			DB::raw("registrasi.nama_dokter as pengirim_pengguna_nama"),
			DB::raw("0 as masuk_kecil"),
			DB::raw("0 as jumlah_kecil"),
			DB::raw("jumlah_kecil as minta_kecil"),
			'resep.created_at',
	...
	$resepBebasQuery = DB::table('resepbebas')
		->select(
			DB::raw("pasienbebas.no_invoice as kode"),
			DB::raw("nama_obat as nama"),
			DB::raw("'Penjualan' as ke_nama_unit"),
			DB::raw("'Apotek' as dari_nama_unit"),
			DB::raw("resepbebas.nama_dokter as pengirim_pengguna_nama"),
			DB::raw("0 as masuk_kecil"),
			DB::raw("0 as jumlah_kecil"),
			DB::raw("jumlah_kecil as minta_kecil"),
			'resepbebas.created_at',
			DB::raw("'keluar' as jenis_transaksi")
		)
		->when($this->obatuuid !== 'empty', function($query) {
			return $query->where('obat_uuid', '=', $this->obatuuid);
		})
		->join('pasienbebas', 'pasienbebas.uuid', '=', 'resepbebas.pasienbebas_uuid')
		->whereBetween('resepbebas.created_at', [$this->dari, $this->ke])
		->where('resepbebas.delete_soft', 1)
		->where('pembayaran', 'Sudah Bayar')
		->where('resepbebas.status', '=', 'active')
		->where('pasienbebas.status', '=', 'active');
	
	$allTransactions = $resepQuery
		->union($resepBebasQuery)
		->union($dataKeluar)
