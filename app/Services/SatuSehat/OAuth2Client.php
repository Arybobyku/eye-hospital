...
use App\Services\Satusehat\Exception\Helper\OAuth2ClientException;
// SATUSEHAT Model & Log
use App\Services\Satusehat\Models\SatusehatLog;
use App\Services\Satusehat\Models\SatusehatToken;
...
class OAuth2Client
{
    ...
    public function __construct()
    {
        ...
        if ($this->satusehat_env == 'PROD') {
            ...
            $this->client_id = getenv('STG_SATU_SEHAT_CLIENT_ID');
            $this->client_secret = getenv('STG_SATU_SEHAT_SECRET_KEY');
            $this->organization_id = getenv('STG_SATU_SEHAT_ORGANIZE_ID');
        } elseif ($this->satusehat_env == 'DEV') {
            $this->base_url = getenv('SATUSEHAT_BASE_URL_DEV') ?: 'https://api-satusehat-dev.dto.kemkes.go.id';
            $this->client_id = getenv('DEV_SATU_SEHAT_CLIENT_ID');
            $this->client_secret = getenv('DEV_SATU_SEHAT_SECRET_KEY');
            $this->organization_id = getenv('DEV_SATU_SEHAT_ORGANIZE_ID');
        }
