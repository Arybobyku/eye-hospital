...
class PasienInapBedahCtrl extends Controller
{
	...
	public function list(Request $request) {
		...
		if ($request->search != "") {
			...
			$total = RegistrasiOperasi::where('delete_soft', '=', 1)
								->where('posisi', '!=', 'Selesai')
								->where('jenis', '=', 'Inap dan Bedah')->where('jenis_pembayaran', '=', 'Umum')->orderBy('id', 'desc')->count();
	...
	public function diterima(Request $request) {
		...
		try{
			...
			$item->kamar_inap_uuid = $data->kamar_inap_uuid;
			$item->kamar_inap_nama = $data->kamar_inap_nama;
			$item->kamar_inap_lantai = $data->kamar_inap_lantai;
			$item->kamar_inap_jumlah_bed = $data->kamar_inap_jumlah_bed;
			$item->jenis_kamar_uuid = $data->jenis_kamar_uuid;
			$item->nama_jenis_kamar = $data->nama_jenis_kamar;
			$item->harga_kamar = $data->harga_kamar;
			$item->keterangan_inap = $data->keterangan_inap;
			...
			$item->jenis_pasien = 'Inap dan Bedah';
			...
			$item->no_pendaftaran = '-';
			$item->registrasi_kode = 'RIDB';
			$item->registrasi_nomor = $nomor_bedah;
			$item->registrasi_jenis = 'Inap dan Bedah';
			...
			$arr = array('status' => 'Inap dan Bedah');
