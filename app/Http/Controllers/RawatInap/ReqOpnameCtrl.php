...
class ReqOpnameCtrl extends Controller
{
	...
	public function list(Request $request) {
		...
		PenggunaHelp::log('(Rawat Jalan) Melihat data list table pada halaman data permintaan obat/alkes pada unit rawat jalan');
		...
		if ($request->search != "") {
			$data = MintaTerimaOpname::where('delete_soft', '=', 1)
				...
				->select( 'kode', 'tanggal_minta', 'jam_minta', 'dari_unit_uuid', 'ke_unit_uuid', 'ke_nama_unit', 'status', DB::raw("count(kode) as jumlah"))
				->groupBy(['kode', 'tanggal_minta', 'jam_minta', 'dari_unit_uuid','ke_unit_uuid','ke_nama_unit', 'status',])
				->where('dari_nama_unit', '=', 'Rawat Inap')
				->orderBy('tanggal_minta', 'desc')
	...
	public function minta(Request $request) {
		...
		if ($request->kode == '') {
			...
			if ($angka > 0 && $angka < 10) { $kode .= '00'.$angka; }
			else if ($angka > 9 && $angka < 100) { $kode .= '0'.$angka; }
			else if ($angka > 99 && $angka < 1000) { $kode .= $angka; }
			PenggunaHelp::log('(Rawat Inap) Melakukan request/permintaan data obat/alkes ke gudang farmasi sebanyak '.count($permintaan).' item obat/alkes dengan kode '.$kode);
		}
		else {
			...
			PenggunaHelp::log('(Rawat Inap) Melakukan perubahan request/permintaan data obat/alkes ke gudang farmasi sebanyak '.count($permintaan).' item obat/alkes dengan kode '.$kode);
		}
		...
		try {
			...
			$unit = Unit::where("nama","=","Rawat Inap")->first();
			...
			foreach($permintaan as $row) {
				...
				$item->dari_unit_uuid = $unit->uuid;
				$item->dari_nama_unit = $unit->nama;
				$item->ke_unit_uuid = 'bc0582ff-ce98-45f4-b361-ecef5b686a0f';
				$item->ke_nama_unit = 'Gudang Farmasi';
	...
	public function detail(Request $request) {
		...
		if ($data) {
			PenggunaHelp::log('(Rawat Jalan) Melihat detail permintaan obat/alkes dengan kode '.$data->kode);
		}
	...
	public function batal(Request $request) {
		...
		$data = MintaTerimaOpname::where('kode', '=', $request->kode)->first();
		if ($data) {
			PenggunaHelp::log('(Rawat Jalan) Membatalkan permintaan obat/alkes ke gudang farmasi dengan kode permintaan '.$data->kode);
		}
	...
	public function terima(Request $request) {
		...
		$data = MintaTerimaOpname::where('kode', '=', $request->kode)->first();
		if ($data) {
			PenggunaHelp::log('(Rawat Jalan) Menerima obat/alkes dari gudang farmasi dengan kode permintaan '.$data->kode);
		}
		...
		try{
			...
			foreach($msg as $row) {
				...
				if ($stockopname) {
					...
					$stockopname->unit_uuid = '60df3cd5-57c2-4f05-b5fa-dffba34400dc';
					$stockopname->nama_unit = 'Rawat Jalan';
					$stockopname->save();
				}
			}
