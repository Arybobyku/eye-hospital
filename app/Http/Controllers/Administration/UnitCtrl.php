...
class UnitCtrl extends Controller
{
	...
	public function list(Request $request) {
		...
		if ($request->search != "") {
			$data = Unit::where('delete_soft', '=', 1)
								->where($column, 'ilike', '%'.$search.'%')
								->orderBy('id', 'desc')
								->skip($skip)->take($this->take)
								->get();
			$total = Unit::where('delete_soft', '=', 1)
								->where($column, 'ilike', '%'.$search.'%')
			...
			$total = Unit::where('delete_soft', '=', 1)->orderBy('id', 'desc')->count();
	...
	public function add(Request $request) { 
		...
		$uuid = ''; $loop = false;
		do { $uuid = Uuid::uuid4(); $check = Unit::where('uuid', '=', $uuid)->first(); if (!$check) { $loop = true; } }while($loop == false);
