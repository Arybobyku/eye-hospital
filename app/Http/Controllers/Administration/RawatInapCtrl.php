...
class RawatInapCtrl extends Controller
{
	...
	public function add(Request $request) {
		...
		if (count($cek) > 0) {
			...
			else if ($cek[0] == 'Honor' || $cek[0] == 'Konsul' || $cek[0] == 'Konsultasi' || $cek[0] == 'Gaji') {
				$item->jenis = 'Honor';
	...
	private function registrasi_jam_selesai($uuid) {
		$arr = array('tanggal_selesai_periksa' => date('Y-m-d'), 'dokter_jam_selesai' => date('H:i'));
	...
	private function registrasi_number($uuid) {
		...
		if ($cek) {
			if ($cek->no_kwitansi == '-' || $cek->no_kwitansi == '' || $cek->no_kwitansi == ' ') {
				...
				if ($nomor_k < 10) { $nomor_k = '0000'.$nomor_k; }
				else if ($nomor_k > 9 && $nomor_k < 100) { $nomor_k = '000'.$nomor_k; }
				else if ($nomor_k > 99 && $nomor_k < 1000) { $nomor_k = '00'.$nomor_k; }
				else if ($nomor_k > 999 && $nomor_k < 10000) { $nomor_k = '0'.$nomor_k; }
