...
use App\Models\FormTindakanOperasi;
...
class DataFormBedahCtrl extends Controller
{
    ...
    public function laporaninjeksiantivega(Request $request)
    {
        ...
        $tindakanOperasi = json_decode($request->listtindakan);
        FormTindakanOperasi::where('registrasi_uuid', '=', $bedah->registrasi_uuid)->delete();

        foreach ($tindakanOperasi as $row) {
            $item = new FormTindakanOperasi();
            $item->uuid = Uuid::uuid4();
            $item->registrasi_uuid = $bedah->registrasi_uuid;
            $item->form_laporan_uuid = $formUuid;
            $item->namatindakan = $row->namatindakan;
            $item->save();
        }
