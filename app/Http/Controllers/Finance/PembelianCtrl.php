...
class PembelianCtrl extends Controller
{
	...
	public function list(Request $request) {
		...
		if ($request->search != "") {
			$data = DB::table('pembelian')
								...
								->get();
			$total = DB::table('pembelian')->where($column, 'ilike', '%'.$search.'%')->where('status', '=', 'approve')->where('pembayaran', '=', 'Kredit')->count();
		}
		else {
			$data = DB::table('pembelian')
								->leftjoin('pembelian_detail', 'pembelian.no_faktur', '=', 'pembelian_detail.no_faktur')
								...
								->skip($skip)->take($this->take)
								->get();
			$total = DB::table('pembelian')->where('pembayaran', '=', 'Kredit')->where('status', '=', 'approve')->count();
	...
	public function listtagihan(Request $request) {
		...
		if ($request->search != "") {
			$data = DB::table('pembelian')
								...
								->skip($skip)->take($this->take)
								->get();
			$total = DB::table('pembelian')->where($column, 'ilike', '%'.$search.'%')->where('status', '=', 'approve')
			...
			$data = DB::table('pembelian')
								...
								->skip($skip)->take($this->take)
								->get();
			$total = DB::table('pembelian')->where('pembayaran', '=', 'Kredit')->where('status', '=', 'approve')
						->whereDate('pembelian.from_tempo', '>=', date('Y-m-d'))
						->where('pembelian.status_pembayaran', '=', 'Belum Dibayar')	
						->count();
	...
	public function cetak(Request $request) {
		$filename = date('Y-m-d').'- Faktur Obat (Pembayaran).xlsx';
		$dari = date('Y-m-d');
	...
	public function ubahstatus(Request $request) {
		...
		$arr = array('status_pembayaran' => 'Sudah Dibayar');
		foreach ($data as $row) {
			$update = DB::table('pembelian')->where('no_faktur', '=', $row->no_faktur)
										->whereDate('tanggal_faktur', '=', $row->tanggal_faktur)
										->update($arr);
		}
	...
	public function add(Request $request) { 
		...
		try{
			...
			if ($request->jangka_waktu != '' && $request->jangka_waktu != '-' && $request->jangka_waktu  && $request->jangka_waktu != '0' ) {
				...
				if ((int) $request->jangka_waktu > 10) {
					$from = (int) $request->jangka_waktu - 10;
					$from_date = $request->tanggal_faktur."+".$from." day";
					$from_tempo = date('Y-m-d', $from_date);
				}
				$date = strtotime($m);
