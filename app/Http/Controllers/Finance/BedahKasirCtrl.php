...
class BedahKasirCtrl extends Controller
{
	...
	public function list(Request $request) {
		...
		if ($request->search != "") {
			$data = Registrasi::join('pasien', 'registrasi.pasien_uuid', '=', 'pasien.uuid')
								...
								->where(function($q){
									$q->where('registrasi.jenis', '=', 'Rawat Inap');
								})
								->orderBy('registrasi.no_kwitansi', 'asc')
								->where(function($q){
									$q->where('registrasi.status', 'Rawat Inap');
								})
	...
	public function getpanjar(Request $request) {
		...
		$panjar = 0; $keterangan = '';
		if ($data) { $panjar = $data->panjar; $keterangan = $data->keterangan_panjar; }
	...
	public function perbaharuibiaya(Request $request) {
		$arr = array('tarif' => $request->tarif, 'total' => $request->total);
	...
	public function call(Request $request) {
		...
		$arr = array('status_antrian_kasir' => '-');
